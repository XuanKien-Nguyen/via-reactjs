...
import {rejectProductRequest} from "../../../../../services/warranty-tickets-manager";
    ...
    const handleSubmit = (detail, loading, setVisible, rerender) => {
        ...
        props.form.validateFields((err, values) => {
            ...
            if (!err && comment) {
                ...
                formData.append('detailProductReject', values.detailProductReject)
                ...
                loading(true)
                rejectProductRequest(detail.id, formData).then(resp => {
                    if (resp.status === 200) {
                        Modal.success({
    ...
    return <Fragment>
        <Form>
            <div style={{border: '1px solid #eaeaea', padding: '20px'}}>
                ...
                {/*</h3>*/}
                <Form.Item label="Danh sách sản phẩm từ chối bảo hành">
                    {getFieldDecorator('detailProductReject', {
                        rules: [{required: true, message: 'Vui lòng nhập danh sách sản phẩm từ chối bảo hành'}],
                    })(
                        <TextArea placeholder={'Danh sách sản phẩm từ chối bảo hành'} rows={8}/>,
                    )}
                </Form.Item>
            ...
            {/*<Button id={'submit-reject-warranty'} type="primary" htmlType="submit" hidden></Button>*/}
        </Form>
    </Fragment>
}
